...
if(isset($_SESSION['user_data'])){
	...
	// } 
	// $qr=mysqli_query($con,"select * from users where id='".$_GET['id']."'");
	...
	if(isset($_GET['id'])){ 
		...
		if(mysqli_num_rows($qr)==0){
			...
			$is_result=false;
			$result_data=array();
			$result_qr=mysqli_query($con,"select * from results where student_id='".$_GET['id']."'");
			if(mysqli_num_rows($result_qr)>0){
				$is_result=true;
				$row=mysqli_fetch_assoc($result_qr);
				//fetching result
				$result_data_qr=mysqli_query($con,"select result_data.*,subjects.subject_name from result_data,subjects where result_data.result_id='".$row['id']."' and result_data.subject_id=subjects.id");
